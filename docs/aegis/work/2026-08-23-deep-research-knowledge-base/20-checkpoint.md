...
- 当前切片：任务 4 — arXiv PDF 与 Hugging Face 材料采集。
- 已完成：设计规格、实施计划、隔离工作区、基线验证；任务 1 的 ORM/schema、`0002_deep_research` 迁移、legacy tag 转换和迁移升降级验证；任务 2 的研究预算、报告引文校验和 AI 研究合同；任务 3 的 GitHub revision-pinned 证据采集。
- 未完成：任务 4–10 的实现与全量验证。
- 下一步：为有文本层 PDF 的页级摘录、无文本/加密/超页 PDF 的 partial 覆盖，以及 Hub 配置/源码选择与权重排除编写失败测试。
