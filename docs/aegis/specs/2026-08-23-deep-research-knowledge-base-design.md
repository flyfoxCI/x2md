...
设置 `research.auto_start` 默认开启。关闭时，来源详情显示“开始深度研究”；开启时，支持的三类来源在成功导入后自动排入同一队列。手动与自动运行使用同一预算、同一编排器和同一输出合同。

状态为 `queued`、`running`、`completed`、`partial`、`blocked`、`failed`。运行中的阶段为 `collecting`、`summarizing`、`reporting` 和 `tagging`。

首期在应用 lifespan 内运行一个持久化、单 worker 的轮询器：数据库中的短期租约保证一个运行只由一个 worker 领取，worker 在长时间采集或模型调用期间持续续租；启动时过期租约回到可领取状态。此版本的部署不支持多个同时启用 worker 的后端副本。证据集合、每条证据笔记和通过引用校验的研究报告都是持久检查点；报告 Artifact 与引用在标签生成前提交，标签阶段失败不能丢弃已经正确生成的研究结果。重试必须从缺失笔记、报告或标签阶段继续，不能重复已完成的采集和分析；若标签请求耗尽所有重试，运行以 `partial` 和 `tag_provider_error` 结束，已验证报告仍可阅读和编辑。单次 AI 请求为正常的长时生成保留充足读取时间，并对超时、网络错误、限流和所有服务端错误执行指数退避；耗尽请求级重试后，worker 仍提供两次持久运行级恢复。受限来源、预算超限、解析错误和未配置 AI 均不盲目重试，并转为 `partial`、`blocked` 或 `failed`，带安全的机器错误码。
...
兼容层只在模型名明确属于 DeepSeek V4 时，对报告与结构化标签请求发送其官方 `thinking: disabled` 控制，并使用更大的报告输出预算；这是为了避免默认高强度推理耗尽全部 completion token 而正文仍为空。其他 OpenAI-compatible 模型不接收该厂商字段，逐证据研究笔记仍可使用模型默认推理行为。标签使用独立的 JSON-only prompt，不复用报告模板；输出最多 12 个证据绑定标签，并允许一次格式修复。若格式修复仍失败但报告已经通过引用校验并持久化，运行以 `partial / invalid_research_tags` 结束，报告继续可用。
