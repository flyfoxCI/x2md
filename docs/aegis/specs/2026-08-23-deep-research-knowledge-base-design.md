...
兼容层只在模型名明确属于 DeepSeek V4 时，对报告与结构化标签请求发送其官方 `thinking: disabled` 控制，并使用更大的报告输出预算；这是为了避免默认高强度推理耗尽全部 completion token 而正文仍为空。其他 OpenAI-compatible 模型不接收该厂商字段，逐证据研究笔记仍可使用模型默认推理行为。
